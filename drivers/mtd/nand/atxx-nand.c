...
static  void nfc_config_big_page(int width, int addr)
{
	if (width == NAND_BUSWIDTH_16) {
		atxx_nd_write_reg(REG_NFC_FADDR0, addr / 2);
		atxx_nd_write_reg(REG_NFC_BADDR, addr / 2);
	} else {
...
{  
	...
	switch (value)
	{
		case 1:
			ecc_number = 8;
			mtd->oobsize = 20 * (mtd->writesize / 512);
			break;
		case 2:
			ecc_number = 12;
			mtd->oobsize = 28 * (mtd->writesize / 512);
			break;
		case 3:
			ecc_number = 10;
			mtd->oobsize = 24 * (mtd->writesize / 512);
			break;
		case 4:
			ecc_number = 15;
			mtd->oobsize = 36 * (mtd->writesize / 512);
			break;
		case 5:
			ecc_number = 6;
...
static int atxx_nd_config_init(struct mtd_info *mtd)
{
	...
	*/
	...
	reg_data = (mtd->writesize + mtd->oobsize)
